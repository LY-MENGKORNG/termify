...
mod tests {
    ...
    /// Reads a reply body, as the client does.
    fn read(json: &str) -> Option<super::Lyrics> {
        serde_json::from_str::<Found>(json)
            .ok()
            .and_then(Found::into_lyrics)
    }

    #[test]
    fn synced_lyrics_are_preferred() {
        let lyrics = read(
            r#"{"syncedLyrics":"[00:01.00] Timed","plainLyrics":"Untimed","instrumental":false}"#,
        );

        assert_eq!(lyrics.as_ref().map(super::Lyrics::is_synced), Some(true));
    }

    #[test]
    fn plain_lyrics_are_used_when_there_are_no_timings() {
        let lyrics =
            read(r#"{"syncedLyrics":null,"plainLyrics":"Untimed words","instrumental":false}"#);

        assert_eq!(lyrics.as_ref().map(super::Lyrics::is_synced), Some(false));
        assert_eq!(lyrics.map(|lyrics| lyrics.lines().len()), Some(1));
    }

    #[test]
    fn an_instrumental_has_nothing_to_show() {
        assert!(read(r#"{"instrumental":true,"plainLyrics":"[Instrumental]"}"#).is_none());
    }

    #[test]
    fn empty_lyrics_count_as_none() {
        assert!(read(r#"{"syncedLyrics":"   ","plainLyrics":""}"#).is_none());
    }

    #[test]
    fn a_reply_missing_every_field_is_not_a_failure() {
        assert!(read("{}").is_none());
    }
}
